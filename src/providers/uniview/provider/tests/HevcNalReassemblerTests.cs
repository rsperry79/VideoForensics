...
namespace VideoForensics.Providers.Uniview.Tests
{
    ...
    public class HevcNalReassemblerTests
    {
        ...
        public void HevcNalReassembler_ProcessSingleNalUnit_WritesProperly()
        {
            ...
            byte[] nal = new byte[] { 0x00, 0xAA, 0xBB, 0xCC }; // NAL header + payload
            byte[] rtpPacket = new byte[RtpHeaderBase.Length + nal.Length];
        ...
        public void HevcNalReassembler_ProcessFragmentedNalUnit_ReassemblesCorrectly()
        {
            ...
            byte[] rtpPacketStart = new byte[RtpHeaderBase.Length + fragmentStart.Length];
            ...
            byte[] rtpPacketMiddle = new byte[RtpHeaderBase.Length + fragmentMiddle.Length];
            ...
            byte[] rtpPacketEnd = new byte[RtpHeaderBase.Length + fragmentEnd.Length];
            ...
            byte[] expectedPayload = new byte[] { 0x62, 0x00, 0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF };
        ...
        public void HevcNalReassembler_IgnoresTooShortPacket()
        {
            ...
            byte[] tooShort = new byte[] { 0x80, 0x6C };
        ...
        public void HevcNalReassembler_IgnoresInvalidRtpVersion()
        {
            ...
            byte[] packet = new byte[RtpHeaderBase.Length + 4];
        ...
        public void HevcNalReassembler_HandlesRtpExtensionHeaders()
        {
            ...
            byte[] nal = new byte[] { 0x00, 0xAA, 0xBB, 0xCC };
            byte[] rtpPacket = new byte[header.Length + nal.Length];
        ...
        public void HevcNalReassembler_MultipleSequentialNals_WritesAllWithStartCodes()
        {
            ...
            byte[] nal1 = new byte[] { 0x00, 0xAA, 0xBB };
            byte[] nal2 = new byte[] { 0x00, 0xCC, 0xDD };

            byte[] packet1 = new byte[RtpHeaderBase.Length + nal1.Length];
            Array.Copy(RtpHeaderBase, packet1, RtpHeaderBase.Length);
            Array.Copy(nal1, 0, packet1, RtpHeaderBase.Length, nal1.Length);

            byte[] packet2 = new byte[RtpHeaderBase.Length + nal2.Length];
            ...
            byte[] expected = new byte[startCode.Length + nal1.Length + startCode.Length + nal2.Length];
            int pos = 0;
            Array.Copy(startCode, 0, expected, pos, startCode.Length);
            pos += startCode.Length;
            Array.Copy(nal1, 0, expected, pos, nal1.Length);
            pos += nal1.Length;
            Array.Copy(startCode, 0, expected, pos, startCode.Length);
            pos += startCode.Length;
