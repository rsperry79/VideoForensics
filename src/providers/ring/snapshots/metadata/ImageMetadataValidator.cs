...
namespace VideoForensics.Providers.Ring
{
    ...
    public class ImageMetadataValidator : IMetadataValidator
    {
        ...
        public bool Validate(string snapshotFilePath)
        {
            ...
            string extension = _fileSystem.Path.GetExtension(snapshotFilePath).ToLowerInvariant();
            string[] validExtensions = new[] { ".jpg", ".jpeg", ".webp", ".png", ".gif" };

            return Array.IndexOf(validExtensions, extension) >= 0 && !IsCorrupted(snapshotFilePath);
        }
        ...
        public bool IsCorrupted(string snapshotFilePath)
        {
            ...
            try
            {
                ...
                string extension = _fileSystem.Path.GetExtension(snapshotFilePath).ToLowerInvariant();
                string? detectedFormat = DetectFormat(snapshotFilePath);
