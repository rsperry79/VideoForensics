...
namespace VideoForensics.Forensics.Tests
{
    ...
    public class EvidenceAccessControllerTests
    {
        private IEvidenceAccessController CreateMockController()
        {
            ...
            mock.Setup(x => x.IsAccessAllowedAsync(It.IsAny<string>(), It.IsAny<string>(), It.IsAny<string>(), It.IsAny<string>()))
                .ReturnsAsync(true);
            mock.Setup(x => x.DetectSuspiciousAccessAsync(It.IsAny<string>()))
                .ReturnsAsync(new List<AccessAnomaly>());
            mock.Setup(x => x.GetAccessHistoryAsync(It.IsAny<string>()))
                .ReturnsAsync(new List<AccessLog>());
            mock.Setup(x => x.FlagAccessForReviewAsync(It.IsAny<string>(), It.IsAny<string>()))
                .Returns(Task.CompletedTask);
            mock.Setup(x => x.GetUserAccessRiskAsync(It.IsAny<string>()))
        ...
        public async Task IsAccessAllowedAsync_WithValidInvestigatorAccess_ReturnsTrue()
        {
            ...
            var result = await controller.IsAccessAllowedAsync("evidence-001", "investigator-001", "investigator", "case-review");
            Assert.True(result);
        }
        ...
        {
            var controller = CreateMockController();
            var result = await controller.IsAccessAllowedAsync("evidence-002", "prosecutor-001", "prosecutor", "trial-preparation");
            Assert.True(result);
        }
        ...
        {
            var controller = CreateMockController();
            var result = await controller.IsAccessAllowedAsync("", "investigator-001", "investigator", "case-review");
        ...
        {
            var controller = CreateMockController();
            var anomalies = await controller.DetectSuspiciousAccessAsync("evidence-clean-001");
        ...
        public async Task DetectSuspiciousAccessAsync_WithFailedAccessAttempts_DetectsAnomaly()
        {
            ...
            {
                new AccessAnomaly { EvidenceId = "evidence-failed-001", AnomalyType = "FailedAccessAttempts", Severity = AccessAnomalySeverity.High, FailedAttemptCount = 5 }
            };
            var mock = new Mock<IEvidenceAccessController>();
            mock.Setup(x => x.DetectSuspiciousAccessAsync("evidence-failed-001")).ReturnsAsync(anomalyList);
            var controller = mock.Object;
            var anomalies = await controller.DetectSuspiciousAccessAsync("evidence-failed-001");
            Assert.NotNull(anomalies);
            var failedAnomaly = anomalies.FirstOrDefault(a => a.AnomalyType == "FailedAccessAttempts");
        ...
        public async Task DetectSuspiciousAccessAsync_WithCriticalSeverity_FlagsForReview()
        {
            var anomalyList = new List<AccessAnomaly>
            {
                new AccessAnomaly { EvidenceId = "evidence-critical-001", AnomalyType = "UnauthorizedModification", Severity = AccessAnomalySeverity.Critical, RequiresInvestigation = true }
            };
            var mock = new Mock<IEvidenceAccessController>();
            mock.Setup(x => x.DetectSuspiciousAccessAsync("evidence-critical-001")).ReturnsAsync(anomalyList);
            var controller = mock.Object;
            var anomalies = await controller.DetectSuspiciousAccessAsync("evidence-critical-001");
            var critical = anomalies.FirstOrDefault(a => a.Severity == AccessAnomalySeverity.Critical);
        ...
        public async Task DetectSuspiciousAccessAsync_WithMultipleAnomalies_ReturnsAll()
        {
            var anomalyList = new List<AccessAnomaly>
            {
                new AccessAnomaly { EvidenceId = "evidence-multi-001", AnomalyType = "FailedAccessAttempts" },
                new AccessAnomaly { EvidenceId = "evidence-multi-001", AnomalyType = "OffHoursAccess" },
                new AccessAnomaly { EvidenceId = "evidence-multi-001", AnomalyType = "RapidAccess" }
            };
            var mock = new Mock<IEvidenceAccessController>();
            mock.Setup(x => x.DetectSuspiciousAccessAsync("evidence-multi-001")).ReturnsAsync(anomalyList);
            var controller = mock.Object;
            var anomalies = await controller.DetectSuspiciousAccessAsync("evidence-multi-001");
        ...
        public async Task GetAccessHistoryAsync_WithAccessHistory_ReturnsAllEntries()
        {
            ...
            {
                new AccessLog { UserId = "investigator-001", AccessGranted = true },
                new AccessLog { UserId = "prosecutor-001", AccessGranted = true }
            };
            var mock = new Mock<IEvidenceAccessController>();
            mock.Setup(x => x.GetAccessHistoryAsync("evidence-001")).ReturnsAsync(history);
            var controller = mock.Object;
            var result = await controller.GetAccessHistoryAsync("evidence-001");
            Assert.NotNull(result);
        ...
        public async Task GetAccessHistoryAsync_WithSuccessfulAndFailedAttempts_ReturnsAllAttempts()
        {
            var history = new List<AccessLog>
            {
                new AccessLog { AccessGranted = true },
                new AccessLog { AccessGranted = false, DenialReason = "Unauthorized" },
                new AccessLog { AccessGranted = true }
            };
            var mock = new Mock<IEvidenceAccessController>();
            mock.Setup(x => x.GetAccessHistoryAsync("evidence-001")).ReturnsAsync(history);
            ...
            Assert.Single(entries, e => !e.AccessGranted);
        }
        ...
        public async Task GetAccessHistoryAsync_EntriesAreInChronologicalOrder()
        {
            ...
            var history = new List<AccessLog>
            {
                new AccessLog { AccessedAt = now.AddHours(-2) },
                new AccessLog { AccessedAt = now.AddHours(-1) },
                new AccessLog { AccessedAt = now }
            };
            var mock = new Mock<IEvidenceAccessController>();
            mock.Setup(x => x.GetAccessHistoryAsync("evidence-001")).ReturnsAsync(history);
            var controller = mock.Object;
        ...
        {
            var controller = CreateMockController();
            var result = await controller.GetAccessHistoryAsync("evidence-no-history-001");
            Assert.NotNull(result);
            Assert.Empty(result);
        }
        ...
        public async Task GetUserAccessRiskAsync_WithCleanHistory_ReturnLowRisk()
        {
            ...
            var mock = new Mock<IEvidenceAccessController>();
            mock.Setup(x => x.GetUserAccessRiskAsync("clean-user-001")).ReturnsAsync(riskProfile);
            var controller = mock.Object;
            var result = await controller.GetUserAccessRiskAsync("clean-user-001");
            Assert.NotNull(result);
        ...
        public async Task GetUserAccessRiskAsync_WithFailedAttempts_IncreaseRiskScore()
        {
            ...
            var mock = new Mock<IEvidenceAccessController>();
            mock.Setup(x => x.GetUserAccessRiskAsync("high-risk-001")).ReturnsAsync(riskProfile);
            var controller = mock.Object;
            var result = await controller.GetUserAccessRiskAsync("high-risk-001");
        ...
        public async Task GetUserAccessRiskAsync_WithHighRisk_ProvidesRecommendedAction()
        {
            ...
            var mock = new Mock<IEvidenceAccessController>();
            mock.Setup(x => x.GetUserAccessRiskAsync("action-user-001")).ReturnsAsync(riskProfile);
            var controller = mock.Object;
            var result = await controller.GetUserAccessRiskAsync("action-user-001");
        ...
        public async Task GetUserAccessRiskAsync_RiskScoreIsBetweenZeroAndOne()
        {
            ...
            var mock = new Mock<IEvidenceAccessController>();
            mock.Setup(x => x.GetUserAccessRiskAsync("risk-user-001")).ReturnsAsync(riskProfile);
            var controller = mock.Object;
            var result = await controller.GetUserAccessRiskAsync("risk-user-001");
        ...
        public async Task GetUserAccessRiskAsync_WithNonexistentUser_ReturnsZeroRisk()
        {
            ...
            var mock = new Mock<IEvidenceAccessController>();
            mock.Setup(x => x.GetUserAccessRiskAsync("nonexistent-999")).ReturnsAsync(riskProfile);
            var controller = mock.Object;
            var result = await controller.GetUserAccessRiskAsync("nonexistent-999");
        ...
        public async Task GetUserAccessRiskAsync_TracksFlaggedAccessReasons()
        {
            ...
            {
                UserId = "flagged-001",
                FlaggedAccessReasons = new List<string> { "Unauthorized modification", "Tampering attempt" }
            };
            var mock = new Mock<IEvidenceAccessController>();
            mock.Setup(x => x.GetUserAccessRiskAsync("flagged-001")).ReturnsAsync(riskProfile);
            var controller = mock.Object;
            var result = await controller.GetUserAccessRiskAsync("flagged-001");
        ...
        {
            var controller = CreateMockController();
            var isAllowed = await controller.IsAccessAllowedAsync("evidence-001", "investigator-001", "investigator", "case-review");
            var history = await controller.GetAccessHistoryAsync("evidence-001");
            var anomalies = await controller.DetectSuspiciousAccessAsync("evidence-001");
            var riskProfile = await controller.GetUserAccessRiskAsync("investigator-001");
        ...
        public async Task AnomalyDetectionAndFlagging_SuspiciousAccess_FlagsForReview()
        {
            var anomalyList = new List<AccessAnomaly>
            {
                new AccessAnomaly { AnomalyType = "RapidAccess", Severity = AccessAnomalySeverity.High }
            };
            var mock = new Mock<IEvidenceAccessController>();
            mock.Setup(x => x.DetectSuspiciousAccessAsync("evidence-anomaly-001")).ReturnsAsync(anomalyList);
            var controller = mock.Object;
            var anomalies = await controller.DetectSuspiciousAccessAsync("evidence-anomaly-001");
            if (anomalies.Any())
            {
