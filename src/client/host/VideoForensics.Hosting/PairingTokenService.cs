...
namespace VideoForensics.Hosting
{
    ...
    public record PairingTokenInfo(string Token, OperatorRole Role, DateTime ExpiresAtUtc, bool IsAnonymousInitiation = false);
    ...
    {
        PairingTokenInfo CreateToken(OperatorRole role, bool isAnonymousInitiation = false);
    ...
    public class PairingTokenService : IPairingTokenService
    {
        ...
        public PairingTokenInfo CreateToken(OperatorRole role, bool isAnonymousInitiation = false)
        {
            PruneExpired();

            string token = Convert.ToBase64String(RandomNumberGenerator.GetBytes(24))
                .Replace('+', '-').Replace('/', '_').TrimEnd('=');
            var info = new PairingTokenInfo(token, role, DateTime.UtcNow + TokenLifetime, isAnonymousInitiation);
            _tokens[token] = info;
            return info;
        }
