...
namespace VideoForensics.Data.Database.Tests
{
    public class DownloadEventRepositoryTests : IAsyncLifetime
    {
        ...
        public async Task DownloadEventRepository_AddAndGet_RoundTrips()
        {
            ...
            DownloadEvent evt = TestDataBuilder.BuildDownloadEvent(deviceId, "evt_001", true);

            await _repository.AddAsync(evt, CancellationToken.None);
            DownloadEvent? retrieved = await _repository.GetAsync(evt.Id, CancellationToken.None);
        ...
        {
            var deviceId = Guid.NewGuid();
            DownloadEvent evt = TestDataBuilder.BuildDownloadEvent(deviceId, "evt_003");

            await _repository.AddAsync(evt, CancellationToken.None);
            DownloadEvent? retrieved = await _repository.GetByProviderEventIdAsync(deviceId, "evt_003", CancellationToken.None);
        ...
        public async Task DownloadEventRepository_GetLatestSuccessfulEventTime_ReturnsMaxOfSuccessfulEvents()
        {
            ...
            DownloadEvent evt3 = TestDataBuilder.BuildDownloadEvent(deviceId, "evt_3", false);
            ...
            DateTime? watermark = await _repository.GetLatestSuccessfulEventTimeAsync(deviceId, CancellationToken.None);

            _ = Assert.NotNull(watermark);
        ...
        public async Task DownloadEventRepository_GetLatestSuccessfulEventTime_IgnoresFailedEvents()
        {
            ...
            DownloadEvent failed1 = TestDataBuilder.BuildDownloadEvent(deviceId, "fail_1", false);
            failed1.EventOccurredAtUtc = baseTime.AddHours(10);
            failed1.DownloadCompletedUtc = null;

            DownloadEvent success1 = TestDataBuilder.BuildDownloadEvent(deviceId, "success_1", true);
        ...
        public async Task DownloadEventRepository_GetLatestSuccessfulEventTime_IgnoresEventsWithoutDownloadCompletedUtc()
        {
            ...
            DownloadEvent incomplete = TestDataBuilder.BuildDownloadEvent(deviceId, "incomplete", true);
            incomplete.EventOccurredAtUtc = baseTime.AddHours(10);
            incomplete.DownloadCompletedUtc = null;

            DownloadEvent complete = TestDataBuilder.BuildDownloadEvent(deviceId, "complete", true);
        ...
        public async Task DownloadEventRepository_GetLatestSuccessfulEventTime_ReturnsNullWhenNoSuccessfulEvents()
        {
            ...
            DownloadEvent evt = TestDataBuilder.BuildDownloadEvent(deviceId, "evt", false);
            await _repository.AddAsync(evt, CancellationToken.None);
        ...
        public async Task DownloadEventRepository_GetByDeviceId_ReturnsAllForDevice()
        {
            ...
            DownloadEvent evt3 = TestDataBuilder.BuildDownloadEvent(otherDeviceId);

            await _repository.AddAsync(evt1, CancellationToken.None);
            await _repository.AddAsync(evt2, CancellationToken.None);
            await _repository.AddAsync(evt3, CancellationToken.None);

            IReadOnlyList<DownloadEvent> list = await _repository.GetByDeviceIdAsync(deviceId, CancellationToken.None);
        ...
        {
            DownloadEvent evt = TestDataBuilder.BuildDownloadEvent();
            await _repository.AddAsync(evt, CancellationToken.None);
        ...
        {
            DownloadEvent evt1 = TestDataBuilder.BuildDownloadEvent();
            DownloadEvent evt2 = TestDataBuilder.BuildDownloadEvent();

            await _repository.AddAsync(evt1, CancellationToken.None);
            await _repository.AddAsync(evt2, CancellationToken.None);

            IReadOnlyList<DownloadEvent> list = await _repository.ListAsync(CancellationToken.None);
            Assert.Equal(2, list.Count);
        }
        ...
        {
            var deviceId = Guid.NewGuid();
            DownloadEvent evt1 = TestDataBuilder.BuildDownloadEvent(deviceId, "dup_evt");
            DownloadEvent evt2 = TestDataBuilder.BuildDownloadEvent(deviceId, "dup_evt");
